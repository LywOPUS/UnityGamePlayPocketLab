...
public class RotateInput : MonoBehaviour
{
    ...
    void RotateTarget3()
    {
        ...
        if (Vector3.Dot(target_dir, -cube.transform.right) > 0)
        {
            cube.transform.Rotate(-Vector3.up * rot * Mathf.Rad2Deg);
        }
        else
    ...
    void RotateByCross()
    {
        Vector3 v = Vector3.Cross(cube.transform.forward, target_dir);
        cube.transform.Rotate(v.normalized*1.0f);
    }
        ...
        if (Mathf.Abs(Vector3.Dot(cube_forward, target_dir)-1) > 0.001f)
        {
            switch (rotate_type)
            {
                ...
                case RotateType.插值:
                    RotateTarget1();
                    break;
                case RotateType.匀速Sin:
                    RotateTarget2();
                    break;
                case RotateType.匀速Cos:
                    RotateTarget3();
                    break;
                case RotateType.直接叉乘法:
                    RotateByCross();
                    break;
            }
        }
